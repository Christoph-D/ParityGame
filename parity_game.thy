...
lemma path_at_in_pset: "P $ n \<noteq> None \<Longrightarrow> the (P $ n) \<in> Pset P"
  apply (induct n arbitrary: P)
   apply (metis Path.set_intros(1) Path.set_intros(3) option.sel path_at_0 path_head.elims)
  by (metis Path.collapse(2) Path.set_sel(2) path_at.simps(2) path_tail_suc)
lemma path_pset_at: "v \<in> Pset P \<Longrightarrow> \<exists>n. P $ n = Some v"
  by (induct v P rule: Path.set_induct, meson path_at.simps(3), metis path_cons_suc_is_P, meson path_at.simps(1))
...
lemma valid_path_impl2: "\<lbrakk> Pset P \<subseteq> V; \<And>i v w. P $ i = Some v \<and> P $ Suc i = Some w \<longrightarrow> v\<rightarrow>w \<rbrakk> \<Longrightarrow> valid_path P" proof (coinduction arbitrary: P rule: valid_path.coinduct)
  case (valid_path P)
  { assume "\<not>(\<exists>v. P = PEnd v \<and> v \<in> V)"
    then obtain v Ps where P: "P = path_cons v Ps" by (metis Path.set_intros(3) path_head.cases subsetCE valid_path(1))
    then obtain w where w: "path_head Ps = w" by simp
    have "v\<rightarrow>w" proof-
      have "P $ 0 = Some v \<and> P $ (Suc 0) = Some w" by (simp add: P w)
      thus ?thesis using valid_path(2) by blast
    qed
    moreover have "Pset Ps \<subseteq> V \<and> (\<forall>i v w. Ps $ i = Some v \<and> Ps $ Suc i = Some w \<longrightarrow> v\<rightarrow>w)" by (metis P Path.set_intros(2) contra_subsetD path_cons_suc_is_P subsetI valid_path)
    ultimately have "\<exists>v w Ps. P = path_cons v Ps \<and> v \<in> V \<and> w \<in> V \<and> v \<rightarrow> w \<and> ((\<exists>P. Ps = P \<and> Pset P \<subseteq> V \<and> (\<forall>i v w. P $ i = Some v \<and> P $ Suc i = Some w \<longrightarrow> v \<rightarrow> w)) \<or> valid_path Ps) \<and> path_head Ps = w"
      using P w edges_are_in_V by auto
  }
  thus ?case by blast
qed
lemma valid_path_equiv: "valid_path P \<longleftrightarrow> Pset P \<subseteq> V \<and> (\<forall>i v w. P $ i = Some v \<and> P $ Suc i = Some w \<longrightarrow> v\<rightarrow>w)" using valid_path_impl1 valid_path_impl2 by blast
...
coinductive maximal_path where
  "deadend v \<Longrightarrow> maximal_path (PEnd v)"
| "maximal_path Ps \<Longrightarrow> maximal_path (path_cons v Ps)"
...
*)
lemma maximal_infinite_path_tail: "\<not>path_deadend P \<Longrightarrow> maximal_path P \<Longrightarrow> maximal_path (path_tail P)"
  by (metis Path.collapse(1) Path.distinct(1) Path.sel(2) maximal_path.cases)
lemma maximal_path_impl1: "maximal_path P \<Longrightarrow> P $ n \<noteq> None \<Longrightarrow> \<not>deadend (the (P $ n)) \<Longrightarrow> P $ Suc n \<noteq> None"
  apply (induct n arbitrary: P)
   apply (metis maximal_path.cases option.sel path_at.simps(1) path_at.simps(4) path_at_0')
  by (metis maximal_path.simps path_at.simps(2) path_at.simps(4))
lemma maximal_path_impl2: "(\<And>n. P $ n \<noteq> None \<and> \<not>deadend (the (P $ n)) \<longrightarrow> P $ Suc n \<noteq> None) \<Longrightarrow> maximal_path P"
proof (coinduction arbitrary: P rule: maximal_path.coinduct)
  case (maximal_path P)
  show ?case proof (cases "\<exists>v. P = PEnd v \<and> deadend v", blast)
    case False
    hence "\<not>path_deadend P" by (metis maximal_path option_the_simp path_2_no_deadend path_at_0 path_at_0' path_deadend_def path_head.simps(2))
    hence "\<exists>v Ps. P = path_cons v Ps" by (metis Path.collapse(1))
    thus ?thesis by (metis maximal_path path_cons_suc_is_P)
  qed
qed
lemma maximal_path_equiv: "maximal_path P \<longleftrightarrow> (\<forall>i. P $ i \<noteq> None \<and> \<not>deadend (the (P $ i)) \<longrightarrow> P $ Suc i \<noteq> None)"
...
lemma path_last_unique:
  "finite_path P \<Longrightarrow> \<exists>!i. P $ i \<noteq> None \<and> (\<forall>j > i. P $ j = None)" by (meson finite_path_equiv nat_neq_iff)
...
lemma path_cons_infinite: "infinite_path P \<Longrightarrow> infinite_path (path_cons v P)" using finite_path.simps by fastforce

(* TODO: probably redundant now *)
lemma path_cons_valid:
  assumes "valid_path P" "P $ 0 = Some w0" "v0 \<rightarrow> w0"
  shows "valid_path (path_cons v0 P)" using assms(1) assms(2) assms(3) valid_path_cons' by auto
...
lemma path_cons_extends:
  ...
  shows "\<exists>i. path_cons v P $ i = Some w" proof-
    ...
    hence "path_cons v P $ Suc i = Some w" by simp
    thus ?thesis by blast
  qed

lemma infinite_path_tail [intro]:
  "infinite_path P \<Longrightarrow> infinite_path (path_tail P)" by (metis Path.collapse(1) infinite_path_no_deadend infinite_path_tail)
lemma finite_path_tail [intro]:
  assumes "finite_path P" "P $ Suc 0 \<noteq> None" shows "finite_path (path_tail P)"
  by (metis Path.sel(2) assms finite_path.cases path_at.simps(2))

lemma valid_path_tail2 [intro]:
  assumes "valid_path P" "P $ Suc 0 \<noteq> None"
  shows "valid_path (path_tail P)" by (meson assms(1) assms(2) path_2_no_deadend valid_path_tail)
